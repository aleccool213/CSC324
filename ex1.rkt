...
(provide search-table search-table-2 max-sublist in-list find-max-starting)

#|
(search-table table item in-table)
...
; could use filter on list-of-lists to return a list where item is in list
; ()
(define (search-table list-of-lists item)
  (filter
    (lambda (x)
      (if (memq item x) #t #f)
    )
...
|#
(define (search-table-2 list-of-lists item position)
  (filter
    (lambda (x)
      (if (> position (- (length x) 1))
        #f
        (if (eq? (list-ref x position) item) #t #f)
      )
    )
  list-of-lists
))
...
; find sum of subarray starting at current element
; only override the value if its more than the current

(define (max-sublist numbers-list)
  (cond
    [(empty? numbers-list) 0]
    [else (
      max
        0
        (find-max-starting 0 numbers-list)
        (max-sublist (rest numbers-list))
      )
    ]
  )
)

(define (find-max-starting last-sum list)
  (cond
    [(empty? list) 0]
    [else
      (max
          ...
          last-sum
          (+ last-sum (first list))
          (find-max-starting (+ last-sum (first list)) (rest list))
      )
    ]
  )
)
